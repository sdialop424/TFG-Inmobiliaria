...
@section('title', 'Editar Incidencia — ComunIpanema')
...
<div class="page-header">
    ...
    <h1 class="page-title"><i class="fas fa-edit"></i> Editar Incidencia</h1>
    <p class="page-subtitle">Modifica los detalles de la incidencia</p>
</div>

<div style="display: grid; grid-template-columns: 1fr; gap: 24px; max-width: 800px;">
    <div class="card">
        <form action="{{ route('incidencias.update', $incidencia) }}" method="POST" novalidate>
            @csrf
            @method('PUT')
        ...
        <select id="propiedad_id" name="propiedad_id"
            ...
            @forelse($propiedades as $propiedad)
                <option value="{{ $propiedad->id }}"
                    {{ old('propiedad_id', $incidencia->propiedad_id) == $propiedad->id ? 'selected' : '' }}>
                    {{ $propiedad->direccion }}
                </option>
            @empty
    ...
    @enderror
                ...
                <div class="form-group">
                    ...
                    <select id="tipo_incidencia_id" name="tipo_incidencia_id" class="form-select {{ $errors->has('tipo_incidencia_id') ? 'is-invalid' : '' }}" required>
                        ...
                        @forelse($tipos as $tipo)
                            <option value="{{ $tipo->id }}" {{ old('tipo_incidencia_id', $incidencia->tipo_incidencia_id) == $tipo->id ? 'selected' : '' }}>{{ $tipo->nombre }}</option>
                        @empty
                ...
                <div class="form-group">
                    ...
                    <select id="estado_incidencia_id" name="estado_incidencia_id" class="form-select {{ $errors->has('estado_incidencia_id') ? 'is-invalid' : '' }}" required>
                        ...
                        @forelse($estados as $estado)
                            <option value="{{ $estado->id }}" {{ old('estado_incidencia_id', $incidencia->estado_incidencia_id) == $estado->id ? 'selected' : '' }}>{{ $estado->nombre }}</option>
                        @empty
            ...
            <div class="form-group">
                ...
                <textarea id="descripcion" name="descripcion" class="form-textarea">{{ old('descripcion', $incidencia->descripcion) }}</textarea>
            </div>
            ...
            <div style="display: flex; gap: 12px; margin-top: 32px;">
                ...
                <a href="{{ route('incidencias.show', $incidencia) }}" class="btn btn-outline">Cancelar</a>
            </div>
        </form>
    </div>
</div>
